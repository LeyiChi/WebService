...
namespace WebService.DataMethod
{
    public class TmpDrugDict
    {
        ...
        // ChangeStatus WY 2015-07-09
        public static bool ChangeStatus(DataConnection pclsCache, string HospitalCode, string DrugCode, string DrugSpec, int Status)
        {
            bool IsSaved = false;
            try
            {
                ...
                int flag = (int)Tmp.DrugDict.ChangeStatus(pclsCache.CacheConnectionObject, HospitalCode, DrugCode, DrugSpec, Status);
                if (flag == 1)
                {
                    IsSaved = true;
                }
            ...
            catch (Exception ex)
            {
                ...
                HygeiaComUtility.WriteClientLog(HygeiaEnum.LogType.ErrorLog, "Tmp.DrugDict.ChangeStatus", "数据库操作异常！ error information : " + ex.Message + Environment.NewLine + ex.StackTrace);
                return IsSaved;
            }
            finally
            {
        ...
        // GetListByStatus WY 2015-07-079
        public static DataTable GetListByStatus(DataConnection pclsCache, int Status)
        {
            ...
            try
            {
                ...
                cmd = Tmp.DrugDict.GetListByStatus(pclsCache.CacheConnectionObject);
                ...
                while (cdr.Read())
                {
                    list.Rows.Add(cdr["HospitalCode"].ToString(), cdr["HospitalName"].ToString(), cdr["DrugCode"].ToString(), cdr["DrugName"].ToString(), cdr["DrugSpec"].ToString(), cdr["Units"].ToString(), cdr["InputCode"].ToString(), cdr["Description"].ToString(), cdr["Status"]);
                }
                return list;
            }
            catch (Exception ex)
            {
                HygeiaComUtility.WriteClientLog(HygeiaEnum.LogType.ErrorLog, "Tmp.DrugDict.GetListByStatus", "数据库操作异常！ error information : " + ex.Message + Environment.NewLine + ex.StackTrace);
                return null;
            }
            finally
            {
