    ...
    public class TmpLabResultDict
    {
        ...
        public static bool SetData(DataConnection pclsCache, string HospitalCode, string Code, string Name, string InputCode, string Description, int Status, string revUserId, string TerminalName, string TerminalIP, int DeviceType)
        {
            ...
            try
            {
                ...
                int flag = (int)Tmp.LabResultDict.SetData(pclsCache.CacheConnectionObject, HospitalCode, Code, Name, InputCode, Description, Status, revUserId, TerminalName, TerminalIP, DeviceType);
                if (flag == 1)
                {
                    IsSaved = true;
                }
            ...
            catch (Exception ex)
            {
                ...
                HygeiaComUtility.WriteClientLog(HygeiaEnum.LogType.ErrorLog, "Tmp.LabResultDict.SetData", "数据库操作异常！ error information : " + ex.Message + Environment.NewLine + ex.StackTrace);
                return IsSaved;
            }
            finally
            {
        ...
        public static bool ChangeStatus(DataConnection pclsCache, string HospitalCode, string Code, int Status)
        {
            ...
            try
            {
                ...
                int flag = (int)Tmp.LabResultDict.ChangeStatus(pclsCache.CacheConnectionObject, HospitalCode, Code, Status);
                if (flag == 1)
                {
                    IsSaved = true;
                }
            ...
            catch (Exception ex)
            {
                ...
                HygeiaComUtility.WriteClientLog(HygeiaEnum.LogType.ErrorLog, "Tmp.LabResultDict.ChangeStatus", "数据库操作异常！ error information : " + ex.Message + Environment.NewLine + ex.StackTrace);
                return IsSaved;
            }
            finally
            {
        ...
        public static DataTable GetListByStatus(DataConnection pclsCache, int Status)
        {
            ...
            try
            {
                ...
                cmd = Tmp.LabResultDict.GetListByStatus(pclsCache.CacheConnectionObject);
            ...
            {
                HygeiaComUtility.WriteClientLog(HygeiaEnum.LogType.ErrorLog, "Tmp.LabResultDict.GetListByStatus", "数据库操作异常！ error information : " + ex.Message + Environment.NewLine + ex.StackTrace);
                return null;
            }
            finally
            {
