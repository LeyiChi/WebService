...
{
    public class MpExaminationItemCmp
    {
        ...
        public static bool SetData(DataConnection pclsCache, string HospitalCode, string HZCode, string Code, string Redundance, string revUserId, string TerminalName, string TerminalIP, int DeviceType)
        {
            ...
            try
            {
                ...
                int flag = (int)Mp.ExaminationItemCmp.SetData(pclsCache.CacheConnectionObject, HospitalCode, HZCode, Code, Redundance, revUserId, TerminalName, TerminalIP, DeviceType);
                if (flag == 1)
                {
                    IsSaved = true;
                }
            ...
            catch (Exception ex)
            {
                ...
                HygeiaComUtility.WriteClientLog(HygeiaEnum.LogType.ErrorLog, "Mp.ExaminationItemCmp.SetData", "数据库操作异常！ error information : " + ex.Message + Environment.NewLine + ex.StackTrace);
                return IsSaved;
            }
            finally
            {
        ...
        public static DataTable GetMpExaminationItemCmp(DataConnection pclsCache)
        {
            ...
            try
            {
                ...
                cmd = Mp.ExaminationItemCmp.GetMpExaminationItemCmp(pclsCache.CacheConnectionObject);
            ...
            {
                HygeiaComUtility.WriteClientLog(HygeiaEnum.LogType.ErrorLog, "Mp.ExaminationItemCmp.GetMpExaminationItemCmp", "数据库操作异常！ error information : " + ex.Message + Environment.NewLine + ex.StackTrace);
                return null;
            }
            finally
            {
